...
const serverRequire = createRequire(
  join(__dirname, '..', 'server', 'node_modules', '_'),
);
...
if (filePaths.length === 0) {
  console.error(
    'Usage: node scripts/import-lesson.js <file.yaml|directory> [--dry-run]',
  );
  process.exit(1);
}
...
async function importLesson(data) {
  ...
  await sql.begin(async (tx) => {
    ...
    console.log(
      `  ✓ Inserted ${usages.length} usages, ${sections.length} sections, ${exercises.length} exercises`,
    );
  });
...
async function importLearningPath(data) {
  ...
  try {
    ...
    if (err.code === '42P01') {
      console.error(
        `  ✗ Table "learning_path" does not exist. Run migration 021 first.`,
      );
      console.error(
        `    Hint: Check database/migrations/ for the learning_path migration.`,
      );
      return false;
    }
    throw err;
