...
export function validateLesson(data) {
  ...
  if (!DIFFICULTY_LEVELS.includes(data.difficulty)) {
    errors.push(
      `lesson.difficulty must be one of [${DIFFICULTY_LEVELS.join(', ')}] (got "${data.difficulty}")`,
    );
  }
  ...
  if (data.usages != null) {
    if (!Array.isArray(data.usages)) {
      ...
      if (data.usages.length > 5) {
        errors.push(
          `lesson.usages may have at most 5 items (got ${data.usages.length})`,
        );
      }
      data.usages.forEach((u, i) => {
        if (!isNonEmptyString(u.title))
          errors.push(`lesson.usages[${i}].title is required`);
        if (!isNonEmptyString(u.description))
          errors.push(`lesson.usages[${i}].description is required`);
      });
    }
  }
  ...
  if (data.sections != null) {
    if (!Array.isArray(data.sections)) {
      ...
      if (data.sections.length > 10) {
        errors.push(
          `lesson.sections may have at most 10 items (got ${data.sections.length})`,
        );
      }
      data.sections.forEach((s, i) => {
        if (!SECTION_TYPES.includes(s.type)) {
          errors.push(
            `lesson.sections[${i}].type must be one of [${SECTION_TYPES.join(', ')}] (got "${s.type}")`,
          );
        }
        ...
        if (!mediaTypes.includes(s.type) && !isNonEmptyString(s.content)) {
          errors.push(
            `lesson.sections[${i}].content is required for type "${s.type}"`,
          );
        }
        if (
          s.metadata != null &&
          (typeof s.metadata !== 'object' || Array.isArray(s.metadata))
        ) {
  ...
  if (data.exercises != null) {
    if (!Array.isArray(data.exercises)) {
      ...
      if (data.exercises.length > 20) {
        errors.push(
          `lesson.exercises may have at most 20 items (got ${data.exercises.length})`,
        );
      }
...
function validateExercise(ex, idx) {
  ...
  if (!EXERCISE_TYPES.includes(ex.type)) {
    errors.push(
      `${prefix}.type must be one of [${EXERCISE_TYPES.join(', ')}] (got "${ex.type}")`,
    );
  }
  ...
  if (!EXERCISE_DIFFICULTIES.includes(ex.difficulty)) {
    errors.push(
      `${prefix}.difficulty must be one of [${EXERCISE_DIFFICULTIES.join(', ')}] (got "${ex.difficulty}")`,
    );
  }
  ...
  if (ex.type === 'multiple_choice') {
    ...
    if (
      ...
      if (correctCount !== 1) {
        errors.push(
          `${prefix}.options must have exactly 1 is_correct=true (got ${correctCount})`,
        );
      }
      ex.options.forEach((opt, j) => {
        ...
        if (opt.label != null && !OPTION_LABELS.includes(opt.label)) {
          errors.push(
            `${prefix}.options[${j}].label must be one of [${OPTION_LABELS.join(', ')}]`,
          );
        }
      });
    }
    ...
    if (ex.correct_answer != null) {
      errors.push(
        `${prefix}: multiple_choice should not have correct_answer (use options[].is_correct)`,
      );
    }
  } else {
  ...
  if (ex.type === 'arrange_words') {
    if (!Array.isArray(ex.word_bank) || ex.word_bank.length === 0) {
      errors.push(
        `${prefix}.word_bank must be a non-empty array for arrange_words`,
      );
    }
  }
  ...
  if (
    ex.points != null &&
    (!Number.isInteger(ex.points) || ex.points < 1 || ex.points > 100)
  ) {
  ...
  if (
    ex.time_limit != null &&
    (!Number.isInteger(ex.time_limit) || ex.time_limit <= 0)
  ) {
...
export function validateLearningPath(data) {
  ...
  if (!isNonEmptyString(data.slug))
    errors.push(`learning_path.slug is required`);
  if (!isNonEmptyString(data.name))
    errors.push(`learning_path.name is required`);
  if (!isNonEmptyString(data.name_vi))
    errors.push(`learning_path.name_vi is required`);
  ...
  if (data.color != null && !HEX_COLOR_REGEX.test(data.color)) {
    errors.push(
      `learning_path.color must be a hex color (got "${data.color}")`,
    );
  }
  ...
  if (!Array.isArray(data.steps) || data.steps.length === 0) {
    ...
    data.steps.forEach((step, i) => {
      if (!isNonEmptyString(step.lesson)) {
        errors.push(
          `learning_path.steps[${i}].lesson is required (lesson slug)`,
        );
      }
    });
  }
